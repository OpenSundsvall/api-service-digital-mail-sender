package se.sundsvall.digitalmail;

import org.springframework.core.io.support.PathMatchingResourcePatternResolver;
import org.springframework.http.MediaType;
import se.gov.minameddelanden.common.X509CertificateWithPrivateKey;
import se.sundsvall.digitalmail.api.model.BodyInformation;
import se.sundsvall.digitalmail.api.model.DigitalMailRequest;
import se.sundsvall.digitalmail.api.model.SupportInfo;
import se.sundsvall.digitalmail.domain.DigitalMailRequestDto;

import java.io.IOException;
import java.security.KeyStore;
import java.security.KeyStoreException;
import java.security.NoSuchAlgorithmException;
import java.security.UnrecoverableEntryException;
import java.security.cert.CertificateException;
import java.security.cert.X509Certificate;
import java.util.List;

public class TestObjectFactory {
    
    public static DigitalMailRequestDto generateDigitalMailRequestDto() {
        
        SupportInfo supportInfo = new SupportInfo();
        supportInfo.setContactInformationEmail("email@somewhere.com");
        supportInfo.setContactInformationPhoneNumber("0701234567");
        supportInfo.setSupportText("support text");
        supportInfo.setContactInformationUrl("http://url.com");
        
        final DigitalMailRequest mailRequest = DigitalMailRequestDto.builder()
                .withPartyId("partyId")
                .withHeaderSubject("Some subject")
                .withMunicipalityId("2281")
                .withBodyInformation(BodyInformation.builder()
                        .withBody("Body text")
                        .withContentType(MediaType.TEXT_PLAIN_VALUE)
                        .build())
                .withAttachments(List.of(createAttachment()))
                .withSupportInfo(supportInfo)
                .build();
    
        final DigitalMailRequestDto mailRequestDto = new DigitalMailRequestDto(mailRequest);
        mailRequestDto.setRecipientId("recipientId");
        return mailRequestDto;
    }
    
    private static DigitalMailRequest.Attachment createAttachment() {
        return DigitalMailRequest.Attachment.builder()
                .withBody(samplePdfBase64)
                .withContentType(MediaType.APPLICATION_PDF_VALUE)
                .withFilename("sample.pdf")
                .build();
    }
    
    public static KeyStore getKeyStore() {
        try {
            KeyStore keyStore = KeyStore.getInstance(KeyStore.getDefaultType());
            keyStore.load(new PathMatchingResourcePatternResolver().getResource("classpath:keystore-junit.p12").getInputStream(), "changeit".toCharArray());
            var privateKeyEntry = (KeyStore.PrivateKeyEntry) keyStore.getEntry("kivra", new KeyStore.PasswordProtection("changeit".toCharArray()));
            final X509Certificate cert = (X509Certificate) privateKeyEntry.getCertificate();
            X509CertificateWithPrivateKey certAndPrivateKey = new X509CertificateWithPrivateKey(cert, privateKeyEntry.getPrivateKey());
            
            return keyStore;
        } catch (CertificateException | UnrecoverableEntryException | KeyStoreException | IOException | NoSuchAlgorithmException e) {
            throw new RuntimeException(e);
        }
    }
    
    private static final String samplePdfBase64 = "";
}
